...
use Think\Log;
class Wechat_payClasslogic extends Classlogic {
    private $id = null;
    ...
    public function __construct($id){
        ...
        if(!$id) $id = 1;
        $this->id = $id;
        $this->distribution_wechat();
    }


    public function distribution_wechat(){
        if($this->id){
            $this->wechat_config = M('distribution_wechat')->where(array('id'=>$this->id))->find();
